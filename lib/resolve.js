...
**/
async function findPath(options , context , moduleName , isTable){
	let filePath = false;
...
**/
async function againstFind(context , filePath , extensions){
	let logogram = filePath.slice(-1) == '/';
	...
	for(let i = tables.length; i --;){
		...
		if(!_path && !s){
			let _file = logogram ? (_filePath + 'index.js') : (_filePath + '/index.js');
			_path = await isPath(path.resolve(_context , _file)) ||
					await isPath(path.resolve(_context , 'node_modules/' + _file));
		}
		if(_path){
...
module.exports = async function (options , context , filePath , parentModule){
	...
	if(nodeLibsBrowser[filePath]){
		...
		if(filePath.indexOf('/') > -1 || path.extname(filePath).length > 1){
			_filePath = await againstFind(context , filePath , options.extensions);
			if(!_filePath){
				let moduleName =  filePath.split('/')[0];
				let table = await findPath(options , context , moduleName , true);
				_filePath = await againstFind(table , filePath , options.extensions);
			}
		} else {
