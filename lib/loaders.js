...
const parseVue = require('./writeVue/loader.js');
...
let coreLoad = [
		{
			...
			loader: async function (module , options){
				let m = parseJavascript.call(this , options , module);
				return m;
			},
			type:'internal'
		},
		{
			test:/\.(css|scss|less)$/,
			loader: async function (module , options){
				let m = await parseSass.call(this , options , module);
					m = await parseLess.call(this , options , module);
					m = parseVue.call(this , options , module);
					m = await parseCss.call(this , options , module);
				return m;
			},
			type:'internal'
		},
		{
			...
			loader: async function (module , options){
				let m = await parseImages.call(this , options , module);
					m = defaultLoader.call(this , options , module);
				return m;
			},
			type:'internal'
		},
		{
			test:/\.vue$/,
			loader: async function (module , options){
				let m = parseVue.call(this , options , module);
				return m;
			},
			type:'internal'
		}
...
async function runLoaders(options , module , loaders){
	...
	for(let i = 0; i <_loaders.length; i ++){
		module = await _loaders[i].loader.call(
			_bale_ , 
			module , 
			_loaders[i].type == 'internal' ? options : _loaders[i].options
		);
	}
...
/**
* 过滤路径 （http）等
* @param { string } filePath  模块引用path
**/
function filterUrl(filePath){
...
async function loader(depTree , options , parentModule , filePath){
	...
	//过滤路径
	if(filterUrl(filePath)){
		return;
	}

	//根据filePath获取并创建模块
	if(type == '[object String]'){
		module = await resolve(options , path.dirname(parentModule.filePath) , filePath , parentModule);
	...
	if(loaders.custom || loaders.default){
		if(loaders.custom){
			...
			module = await loaders.default.loader.call(_bale_ , module , options);
		}
	} else {
	...
	//触发单个module构建完成事件
	await _bale_.emit('singleModule' , module);
