...
function createdModule(filePath){
	...
	let module = {};
		...
		//导出的文件名
		module.outputName = null;
		//模块的绝对路径 或这个模块不变的唯一标识
		module.filePath = filePath;
		//模块文件后缀
		module.suffix = path.extname(filePath);
		//模块依赖关系
		module.depends = {
			import : [],
			export : []
...
async function findPath(options , context , moduleName){
	let filePath = await resolvePath(path.resolve(context , moduleName));

	//如果在当前context下找到则直接返回路径 否则递归node_modules
	if(filePath){
		return filePath;
	} else {
		...
		for(let i = tables.length; i --;){
			if(tables[i]){
				...
				if(package && package.main){
					filePath = path.join(_context , './node_modules/' + moduleName , package.main);
					break;
				} else {
