...
module.exports = function (depTree , options){
	...
	//遍历模块 拼接到output.js
	spliceChunks(options , depTree.root.main);

	let outputSource = template.replace('/entry-bale-id/' , depTree.root.main.id).replace('/bale-chunks/' , chunks);

	//是否压缩代码
	if(options.internalLoaders.uglifyJs){
		outputSource = UglifyJS.minify(outputSource).code;
	}
	return outputSource;
}